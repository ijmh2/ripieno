...
async function main() {
  const ctx = await esbuild.context({
    ...
    entryPoints: ["src/extension.ts", "src/permissionServer.ts", "src/workspaceFs.ts", "src/workspaceServer.ts", "src/addressing.ts", "src/soloRelay.ts", "src/invite.ts", "src/relaySecurity.ts", "src/agentHost.ts", "src/runners.ts", "src/approvalScope.ts", "src/approvalSummary.ts", "src/roomViewMessages.ts", "src/agentSetup.ts", "src/agentCommands.ts", "src/goalCommands.ts", "src/goalState.ts", "src/goalMutations.ts", "src/handoffCommands.ts", "src/handoffState.ts", "src/sessionScope.ts"],
    bundle: true,
    format: "cjs",
    platform: "node",
    target: "node18",
